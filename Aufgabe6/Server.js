...
const Http = require("http"); //creating a HTTP object in the code, interpreter looks for each possible import in the http module and append it to the http object in the code
var L05_Server;
(function (L05_Server) {
    console.log("Starting server"); //obvious
    let port = Number(process.env.PORT); //sets a port as a number (previously a string, converted to a number). If port from user matches the port variables number, they can "communicate" (I think thats how ports work?).
    ...
    let server = Http.createServer(); //variable "server" is declared here and is assigned the "Http.Server" characteristik by the import function in row 1. It is told to create a Http server
